...
int main(){
	...
	std::fstream plik;
	plik.open("day4_input.txt", std::ios_base::in);
	...
	//std::cout<<v.size()<<" "<<v[0].size()<<" "<<v[0][0].size();
			...
			for(int k=0; k<v[0][0].size(); k++){
				if(v[i][j][k]==bingo_numbers[z]){					
					...
					if(v[i][j][0]+v[i][j][1]+v[i][j][2]+v[i][j][3]+v[i][j][4]==-5 || v[i][0][k]+v[i][1][k]+v[i][2][k]+v[i][3][k]+v[i][4][k]==-5){
						//std::cout<<v[i][j][0]+v[i][j][1]+v[i][j][2]+v[i][j][3]+v[i][j][4]<<" "<< v[i][0][k]+v[i][1][k]+v[i][2][k]+v[i][3][k]+v[i][4][k]<<"\n";
						memi=i;
